import numpy as np
from pysnr.utils import (
    mag2db,
    _remove_dc_component,
    bandpower,
    periodogram,
    _get_tone_indices_from_psd,
)
...
def sinad_power_spectral_density(pxx, frequencies):
    ...
    if not pxx_dataCheck or not frequenciesCheck:
        raise TypeError(
            "Power Spectral Density data and Frequency List must be 1-D arrays"
        )
    if len(f) != len(pxx):
        raise AssertionError(
            "Power Spectral Density data and Frequency List must be of same length"
        )
...
def sinad_power_spectrum(sxx, frequencies, rbw):
    ...
    if len(f) != len(sxx):
        raise AssertionError(
            "Power Spectrum data and Frequency List must be of same length"
        )
    pxx = sxx / rbw
    return sinad_power_spectral_density(pxx, f)
